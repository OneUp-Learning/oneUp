        ...
        if request.method == 'GET':
            ...
            for studentobj in students:
                ...
                if studentobj.studentID.isTestStudent:
                    test_students_details.append(
                        {'id': studentobj.studentID, 'name': f'{name} (Test Student)'})
                else:
                    students_details.append(
                        {'id': studentobj.studentID, 'name': name})

            test_students_details = sorted(
                test_students_details, key=lambda x: x['name'].casefold())
            ...
            for badge in badges:
                if badge.manual == True:
                    manual_badges.append(
                        {'id': badge.badgeID, 'name': badge.badgeName, 'description': badge.badgeDescription})
                elif badge.manual == False and badge.isPeriodic == False:
                    automatic_badges.append(
                        {'id': badge.badgeID, 'name': badge.badgeName, 'description': badge.badgeDescription})
                elif badge.isPeriodic == True:
                    periodic_badges.append(
                        {'id': badge.badgeID, 'name': badge.badgeName, 'description': badge.badgeDescription})
            ...
            for student in students_details:
                studentBadges = StudentBadges.objects.filter(
                    studentID=student['id'], badgeID__courseID=course)
                if studentBadges.exists():
                    badges = []
                    for badge in studentBadges:
                        badges.append(
                            {'id': badge.studentBadgeID, 'name': badge.badgeID.badgeName, 'image': badge.badgeID.badgeImage})
                    student_badges_details.append(
                        {'student': student, 'badges': badges})
            ...
            if 'userID' in request.POST:
                ...
                studentID = User.objects.filter(
                    username=request.POST['userID']).first()
                student = Student.objects.filter(user=studentID).first()

                # create the badge in the student section
                # save it in
                studentBadge = StudentBadges()
                referencedBadge = BadgesInfo.objects.filter(
                    badgeID=request.POST['badgeID']).first()
                ...
                register_event(Event.badgeEarned, request,
                               student, referencedBadge.pk)
                notify.send(None, recipient=student.user, actor=student.user, verb=f'You won the {referencedBadge.badgeName} badge', nf_type='Badge', extra=json.dumps(
                    {"course": str(course.courseID), "name": str(course.courseName), "related_link": '/oneUp/students/StudentCourseHome'}))
