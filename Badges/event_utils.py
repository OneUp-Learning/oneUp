def updateLeaderboard(course):
    ...
    for student in students:
        xp = studentXP(student.studentID, course)
    ...
    studentLeaders = sorted(studentLeaders.items(), key=lambda d: d[1], reverse=True)
    print(str(studentLeaders))
        
    # Get the maximum students displayed on leaderboard
    maxPositions = CourseConfigParams.objects.get(courseID = course).numStudentsDisplayed
    currentPosition = 1
    for student, xp in studentLeaders:
        # See if the student is currently active on the leaderboard
        studentLeaderboardData = StudentLeaderboardHistory.objects.filter(courseID = course, studentID = student.studentID, endTimestamp=None)
