...
@login_required
def classSeriousAchievementsViz(request):
            ...
            if StudentChallenges.objects.filter(courseID=currentCourse, challengeID = challenges[t]) :                    
                for i in range(0,num_usercount):
                    if StudentChallenges.objects.filter(studentID= user[i], courseID=currentCourse, challengeID = challenges[t]):
                        ...
                        earliestChall =  StudentChallenges.objects.filter(studentID=user[i], courseID=currentCourse, challengeID = challenges[t]).earliest('startTimestamp')
                        ...
                        if not studentSkills1:
                            ...
                            skill_pointsUserTotal1.append(0)    # DD 09/14/2016
                        else:
                        ...
                        print('userScores: '+str(userScores))
                        print('userEarliestScores: '+str(userEarliestScores) )
                        print('maxTestScores: '+ str(maxTestScores)  )
                        print('minTestScores: '+ str(minTestScores))
        ...
        for i in range(0, num_users):     
            ...
            for j in range(0, num_challs):  
                if StudentChallenges.objects.filter(studentID=user[i], courseID=currentCourse, challengeID = challenges[j]) :
                    ...
                    highestSC = StudentChallenges.objects.filter(studentID=user[i], courseID=currentCourse, challengeID = challenges[j]).order_by('startTimestamp')[:1] 
                    print("earliestSC: "+str(earliestSC))
                    print("latestSC.testScore: "+ str(latestSC.testScore))
                    #print("highestSC"+ str(highestSC.testScore))
                    mediumRecordNumber = str(round(sChallenges.order_by('startTimestamp').count()/2))
                    print("sChallenges: "+ str(sChallenges.order_by('startTimestamp').count()))
                    
                    if sChallenges.order_by('startTimestamp').count() == 1:
                       mediumRecordNumber = 1 
                    else:
                       mediumRecordNumber =  round(sChallenges.order_by('startTimestamp').count()/2)
                    #print("schallengesCountby2 : "+str(round(sChallenges.order_by('startTimestamp').count()/2)))
                    print("schallengesCountDivideby2 : "+str(mediumRecordNumber))
                    mediumSC = StudentChallenges.objects.filter(studentID=user[i], courseID=currentCourse, challengeID = challenges[j]).order_by('startTimestamp')[:mediumRecordNumber]
                    print("mediumSC: "+ str(mediumSC))
                    print("sChallenges: " + str(sChallenges))    
                    ...
                    earliestTestScore = str(earliestSC.testScore)
                    print ("latestTestScore"+latestTestScore)
                    print ("earliestTestScore"+earliestTestScore)
                                   
                    print('gradeMax: '+ str(gradeMax.pop()))
                    print('gradeMin: '+ str(gradeMin))
                    challengeGrade.append(zip(challengesNames,str(user_Name),latestTestScore,earliestTestScore,str(skill_pointsTotal)))      
    ...
    return render(request,'Instructors/ClassSeriousChallengesAchievementsViz.html', context_dict)
