...
#    context_dict["classFund"] = ins_cou.Donations
    ...
    context_dict['studentBadges'] = studentBadges(
...
def progressBarData(request):
    ...
    if request.method == 'GET':
        ...
        if 'class_scores' in request.GET and 'metric_average' in request.GET:
            ...
            metric_average = json.loads(
                request.GET.get('metric_average', 'false'))

        response = progress_bar_data(
            currentCourse, context_dict['ccparams'], class_scores=class_scores, metric_average=metric_average, for_student=context_dict['student'])
    else:
...
def progress_bar_data(current_course, ccparams, class_scores=None, metric_average=None, for_student=None):
    ...
    if class_scores:
        students = StudentRegisteredCourses.objects.filter(
            courseID=current_course, studentID__isTestStudent=False)
        ...
        if for_student:
            data = studentScore(for_student, current_course,
                                0, result_only=True)
            response['data'] = data
...
def studentBadges(currentCourse, student=None):
    ...
    if st_crs:
        if currentCourse:
            ...
            badges = StudentBadges.objects.filter(
                badgeID__courseID=currentCourse).order_by('-timestamp')
